...
def main():
    ...
    result_files = list(results_dir.glob('codellama_code_results_*.pickle'))
    
    if not result_files:
        print("未找到CodeLlama模型结果文件")
        return
    
    print(f"找到 {len(result_files)} 个CodeLlama模型结果文件")
    ...
    print(f"分类结果已保存至 'results/codellama_classifier_results.pickle'")
